...
def sowd(traj_1, traj_2, precision=7, converted=False):
    ...
    if converted:
        ...
        cells_list, cells_traj_list, _, _, _, _, = linec.trajectory_set_grid([traj_1, traj_2], precision)
        # print(np.array(cells_list[0])[:, :2])
        # print(np.array(cells_list[0]))
        # print(np.array(cells_list[1])[:, :2])
        # print('dsdsd', cells_traj_list[0][-1], cells_traj_list[0][-2])
        # print(cells_traj_list)
        if cells_traj_list[0][-1] == cells_traj_list[0][-2] and cells_traj_list[1][-1] == cells_traj_list[1][-2]:
            cells_traj_list = [cells_traj_list[0][: len(cells_traj_list[0])-1], cells_traj_list[1][: len(cells_traj_list[1])-1]]

        d = sowd_grid(np.array(cells_traj_list[0])[:, :2], np.array(cells_traj_list[1])[:, :2])
        # d = sowd_grid(np.array(cells_traj[0]), np.array(cells_traj[1]))
    return d
