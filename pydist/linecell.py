...
def linecell_lons_bigger_step(p1, p2, cell_start, lons_all, lats_all, lons_center_all, lats_center_all):
    ...
    if reverse:
        cell.reverse()
    # print('cell_start: ', cell_start)
    # print(lons_all, lons_center_all)
    # print(lats_all, lats_center_all)
    # print(cell)
    # cells_coord = [[lons_center_all[x[0]], lats_center_all[x[1]]] for x in cell]
    cells_coord = []
    for x in cell:
        try:
            cells_coord.append([lons_center_all[x[0]], lats_center_all[x[1]]])
        except:
            continue
    ...
    for traj in traj_set:
        ...
        for id_seg in range(len(traj) - 1):
            ...
            if time:
                ...
                cell_time2 = cell_time2 + [x + [False] for x in cells_coord[1:-1]]
            # print('cell time', cell_time)

            cells.extend(cell_time)
            cells_coords.extend(cell_time2)
            cell_start = cell[-1]
            cell_start2 = cells_coord[-1]
        if time:
            cell_end_time = end[2]
            ...
            if cell_start != cells[-1][:2]:
                ...
                cells_coords.append(cell_start2 + [True])
                # print('dsseeeeed', cell_start2 + [True])
        cells_traj.append(cells)
        # print(cells_coords)
        cells_traj2.append(cells_coords)
    # cells_traj_=map(np.array,cells_traj)
    cells_traj1_new = []
    cells_traj2_new = []
    for cell_traj1 in cells_traj2[0]:
        if cell_traj1 not in cells_traj1_new:
            cells_traj1_new.append(cell_traj1)
    for cell_traj2 in cells_traj2[1]:
        if cell_traj2 not in cells_traj2_new:
            cells_traj2_new.append(cell_traj2)
    return cells_traj, [cells_traj1_new, cells_traj2_new], lons_all, lats_all, lons_center_all, lats_center_all
